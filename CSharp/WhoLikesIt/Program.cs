...
public class Kata
{
    public static string Likes(string[] name)
    {
        ...
        // return SolutionOne(name);
        return SolutionTwo(name);
        ...
        */
        static string SolutionTwo(string[] name)
        {
            string[] patterns = [
                "no one likes this",
                "{0} likes this",
            ...
            string selectedPattern = patterns[Math.Min(name.Length, 4)];
            return name.Length > 3 ? string.Format(selectedPattern, name[0], name[1], name.Length - 2) : string.Format(selectedPattern, name);
        }

        /**
         * Flat switch and replace.Intuitive.
         */
        static string SolutionOne(string[] name)
        {   
            switch (name.Length)
            {
                case 0:
